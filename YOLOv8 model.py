...
def train_yolov8(dataset, epochs=20, batch_size=16, pretrained_model=None):
    """
    Train YOLOv8 on the prepared dataset, optionally starting from a pretrained model.
    ...
    # Load YOLOv8 model (optionally using a pretrained model)
    model = YOLO(pretrained_model if pretrained_model else "yolov8s.pt")

    # Debug: Verify the model is loaded
    print(f"Model {pretrained_model if pretrained_model else 'yolov8s.pt'} loaded successfully.")
    ...
    model.save("yolov8_garbage_detection_model.pt")
    print("Model saved as 'yolov8_garbage_detection_model.pt'.")
...
if __name__ == "__main__":
    try:
        # Prepare the dataset
        dataset = prepare_yolov8_dataset()

        # Train the model
      #  train_yolov8(dataset, epochs=30, batch_size=16)

        # Load the trained model
        model = YOLO("yolov8_garbage_detection_model.pt")
